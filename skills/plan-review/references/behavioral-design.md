...
Planning artifacts, reflection, memory, retries, walkthroughs, screenshots, orchestration, evaluators, and multiple agents are mechanisms rather than outcomes. Require one only when it closes a demonstrated behavioral gap or controls an identified material failure. Prefer direct behavior and existing enforcement when they satisfy the same obligation.
...
The proposal must specify enough to establish how pivotal instructions and facts become available, how conflicts or precedence are resolved, which capability can act, and which boundary determines the result. Plan Review may inspect current composition and behavior as evidence, but whether an exact future candidate is actually loaded and behaves correctly belongs to delivery acceptance.
...
This includes language, classification, generation, perception, or interpretation by a model. Test whether the design controls unsupported completion, anchoring on proposal examples, context sensitivity, unstable decisions, untrusted content, and evidence that no longer applies after a material model, configuration, or composition change. Do not turn nondeterminism into a requirement for voting, memory, or repeated runs unless the retained guarantee needs them.
