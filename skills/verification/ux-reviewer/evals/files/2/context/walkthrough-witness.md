...
- Starting state: user details for `Alex Rivera` with email `alex@example.com`.

## Recorded path

1. Choose the visible `Send password reset` action.
2. A dialog appears with title `Send password reset link?`.
3. Its body says: `Send a password reset link to alex@example.com. The link expires in 30 minutes. This does not change the password until Alex uses the link.`
4. The primary action is `Send reset link`; the secondary action is `Cancel`.
