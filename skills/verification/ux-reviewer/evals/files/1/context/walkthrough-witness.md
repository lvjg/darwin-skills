...
- Starting state: workspace list contains `Atlas`, `Atlas QA` and `Atlas Archive`; `Atlas QA` is selected.

## Recorded path

1. Open `Atlas QA` settings.
2. Choose the visible `Delete workspace` action.
3. A modal appears with title `Delete`, body `This action cannot be undone.`, primary button `Continue`, and secondary button `Cancel`.
4. The modal does not display `Atlas QA` or any other workspace identifier.
