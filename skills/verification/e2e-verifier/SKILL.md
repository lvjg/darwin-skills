...
推导 case 时先明确整体旅程义务和本轮范围；仅推导用例不要求完整实现或最终候选。执行时把 witness 绑定到实际验收对象：适用时包括 revision/build、部署标识、决定行为的配置、Provider 版本及接受基线。缺少标识时记录替代权威证据和结论边界，不能只用环境名称近似绑定候选。
...
删除或合并不能增加判别力的 case。字段、状态和算法穷举由生产 Owner 的聚焦测试承担；E2E 保留会改变跨边界结果的代表性组合，引用正式 schema 或 conformance 证据。缺少证据时记录缺口，不手抄穷举来伪造闭合。用例质量随推导和执行核实，不自动增加独立审查轮次。
...
故障与恢复 case 要证明条件实际命中目标边界并进入待验证分支；配置了故障或重放成功不证明该分支执行。以业务 correlation 对账实际效果的身份、内容和允许次数，不能只把两个可能同时重复的中间记录集合相互配对。

不要为了方便调用内部写接口、伪造外部成功事件或直接修改终态。结果不明的写操作先查询权威状态，不盲目重试。同一写入仍在活动或终态未知时不得启动冲突或重复动作。

## 按证据判定和局部调整

在关键结果点按短循环推进：绑定当前证据 → 对照 Oracle 判定 → 定位偏差与受影响范围 → 调整并验证受影响部分 → 决定继续或停止。
...
Verifier 不修改产品候选。产品缺陷交给对应 Owner。授权覆盖时可修复 Verifier 的断言、driver 或脚手架，但不得改变业务 Oracle 来制造绿色。只读观测或证据解析修复，可在仍匹配候选与前置事实的原实例补证；改变真实动作、故障注入或决定行为的条件时，重跑受影响入口。保留原 witness、修正后的归因及证据适用边界。

故障注入和临时状态由使用它的验证作用域负责释放，提前失败也不能依赖后续用例清理。需要脱敏或删除运行资料时，按已授权的对象与字段处理，不对所有可读文件作无差别字符串替换；清理后确认仍用于判定的证据身份、关联和候选绑定未被改写，删除的原始资料不再作为可读取的恢复引用。
...
输出先列旅程与验收范围、三个门禁、决定性证据和未完成项；执行时明确实际候选、环境及真实与 mock 边界，仅推导用例时不要求尚不存在的最终候选或运行环境。交付用例集时给出覆盖依据、case 和 Oracle；能力组合、归因、证据失效、重跑及清理细节仅在影响结论、发生变化或必要交接时展开。增量复验引用仍有效的既有依据，明确本次变化及其影响；未知效果和未清理副作用必须披露。不要把结构检查、源代码推断、mock、低层测试或功能之外的产品走查写成具体旅程已经通过。
