...
Use this standard only to express the judgment completed in `SKILL.md`. Do not decide the goal, route, Key Design eligibility, ownership, state, or failure semantics while drafting. Return an exposed gap to the stage that owns it instead of inventing a mechanism through document structure.
...
**Reconcile revisions.** For completion or rewrite, compare the artifact with the latest revision anchor, still-material existing content, and the working change plan when used. Confirm that valid mechanisms and evidence were preserved, superseded machinery was removed, and the semantic change surface covers applicable facts and triggers, contracts and consumers, information and state, trust and permissions, effects, recovery, observability, deployment, migration, rollback, and acceptance. Keywords and a complete table of contents do not prove preservation.
