...
The artifact has one of two skeletons, decided by the requested scope. A whole-system artifact uses Overall Design and includes Key Designs only for mechanisms established as independently design-significant. A key design or local part of an existing design uses a local skeleton: an opening paragraph stating the local judgment, followed only by the inputs, outputs, responsibility boundary, and handoffs to the surrounding system that the judgment depends on; a flow or a sub-section appears only when it changes the judgment. The local skeleton references the adopted surrounding design where that governs and does not build an Overall Design or Key Designs to satisfy the template. Rules below that name Overall Design or Key Designs apply to the whole-system skeleton; the local skeleton is governed by this paragraph and by the Drafting Checks.
