...
Before editing, use focused checks when needed to establish baseline behavior, attribute an existing failure, or decide whether a proposed simplification is valid. Unless their evidence is needed to choose the route, defer expensive or broad validation until the relevant candidate is stable and necessary simplification is complete, then rerun checks invalidated by the final edits. Side effects follow the task's authorization and cleanup requirements at either stage. This timing does not waive required checks.

Use the smallest validation that distinguishes success from failure. Prefer an existing contract, real consumer, boundary execution, or discriminating counterexample when candidate-authored tests merely restate the implementation. Include data preservation, ordering, concurrency, idempotency, cancellation, resource lifetime, security, performance, migration, or external validation only when the candidate exposes those risks.
