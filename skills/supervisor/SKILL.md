...
检查点是 Supervisor 私有的当前控制状态。只有 Supervisor 读取和更新；Worker 不读取、不写入，也不需要知道检查点或版本的存在。检查点表示一个当前有效快照；沿用已有稳定载体及其可用格式，只有现有表示妨碍恢复判断时才调整。

检查点及其中引用的权威来源必须足以让恢复后的 Supervisor 确定：

- 目标结果、验收边界、授权、禁止效果和必须保持行为；
- 直接决定当前路线的权威、设计和对象引用，以及可变对象的必要版本；
- 当前控制状态、所有已成立且完成前必须闭合的义务，以及当前选中的义务；
- 仍影响下一动作、恢复判断或完成判定的有效结论和证据引用。

只保留删除后会改变下一动作、恢复判断或完成判定的当前有效信息。设计决定当前路线时直接记录 `design ref`，设计可变时再附版本；可以用稳定引用代替内容复制，但引用必须可定位、可读取并足以区分当前依据与旧依据。

根据当前状态记录：

- Active：不存在已经被接受但结果未知的动作；下一项动作已经通过 Guard 时，再记录其责任、输入引用、作用边界、预期结果和停止条件；
...
Complete 检查点应指向已验收的具体产物版本：记录产物引用以及可区分该版本的 revision、commit 或 diff 依据。不要用文件大小、行数或修改时间标识版本，也不要复制可从引用重新读取的内容。

不要求通用 action ID、runtime ref 或 Worker 身份协议；领域 Owner 返回恢复查询必需的稳定标识时，把它作为领域对象引用保存。

更新时只改变发生变化的控制信息，保留仍有效内容，并删除已失效或互相冲突的内容。权威、设计或对象依据改变时，同时删除或替换依赖旧依据的结论、动作和证据；没有因果依赖的内容继续保留。载体可以通过局部修改或整体覆盖保持当前快照，不要求迁移已有表示格式。检查点不是活动日志、修订历史、证据数据库、完整交接内容或会话副本；不要保存完整提示词、已失效候选、旧结果或每一步经过。
...
如果中断发生在动作已经可能产生效果、但对应 Waiting 状态尚未落盘的窗口，恢复时把动作效果视为未知，不得依据旧 Active 快照直接重试。`Needs Authority`、`Needs Input` 等是 Blocked 的原因，不是额外状态；不要使用 Deferred 隐藏未闭合义务。

恢复时由 Supervisor 先读取检查点，再重新读取可能变化的权威来源。只有任务需要等待、阻塞或以后恢复时，才对用户提供稳定 checkpoint ref、载体已有版本、控制状态和下一动作、等待条件或阻塞原因；交还仍为 Active 的恢复任务时，应在回复中明确下一动作，不能只把它保存在检查点里。完成时按领域结果和证据报告。不要为内部更新输出检查点。
