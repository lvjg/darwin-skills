...
- The Controller may update only `checkpoint.md`. One ordinary implementation
  Worker may update only `lib/policy_config.rb` and
  `test/policy_config_test.rb`, run the focused Ruby test, and return its bound
  diff and test evidence.
- Complete this task only after the ordinary Worker return is bound to the
  dispatched action and input checkpoint revision and the focused proof closes
  the accepted implementation obligation.
