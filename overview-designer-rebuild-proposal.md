...
本文保留重构时的设计依据、候选正文及行为评测计划，是历史方案。当前入口、正文和配套资源以 [System Designer](skills/design/system-designer/SKILL.md) 及其引用文件为准；下文的旧名称、拟议迁移步骤和候选内容不作为当前执行规范。仓库中的重构已完成，实际行为收益和对照采用条件尚未验证。
